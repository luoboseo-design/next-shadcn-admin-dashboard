...
export function CreateTaskForm({ selectedPackageId }: CreateTaskFormProps) {
  ...
  const handlePlatformTypeChange = (type: PlatformType, checked: boolean) => {
    setFormData((prev) => ({
      ...prev,
      platformTypes: checked
        ? [...prev.platformTypes, type]
        : prev.platformTypes.filter((t) => t !== type),
    }));
      ...
      {selectedPackage && (
        <div className="p-4 rounded-lg bg-muted/50 border">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-lg font-bold">${selectedPackage.totalPrice}</span>
          </div>
          <div className="text-sm text-muted-foreground">
            {selectedPackage.quantity} 条外链 · ${selectedPackage.pricePerLink}/条
          </div>
        </div>
      )}
